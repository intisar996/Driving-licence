...
<?php
	...
	$result = mysqli_query($con,$qryname);
	while($rows = mysqli_fetch_assoc($result)) 
	{ $name=$rows['trainee_fname'];
	


    include('connect.php');
$q="select * from   tbldetailstrainng a,  tblselecttraineer b , tbltrainee c where a.deid=b.deid and  b.trainee_username=c.trainee_uname  and  c.trainee_uname='$usernam'   ";
...
if($num==0)
{
 echo "<h3>you are not selected trainer yet  </h3>";
}
else
{
...
echo"<th class='border border-info'>  Your User Name";
...
echo"<td class='border border-info'><a href='Moreinfo_trainer.php?deid=$deid&&usernam=usernam'>MORE DETAILS</a>";
